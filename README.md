...
## Installation
Our web-app is dependent upon React, Node.js, NetworkX, and Sigma.js libraries. Therefore, you will need run the following commands once you have cloned the repository. If you don't have npm installed, please [Click this link](https://docs.npmjs.com/downloading-and-installing-node-js-and-npm) for more information. Run the following command to install dependencies

### `npm install`
Once all dependencies are installed, run the following command to start both the server and front end locally:

### `npm start`
We use [concurrently](https://www.npmjs.com/package/concurrently) to build and run both the client and the server using a single command (npm start). For more information about the commands that run under the hood, check out the package.json file under scripts.
